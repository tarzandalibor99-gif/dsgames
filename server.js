...
const httpServer = http.createServer((req, res) => {
  // Servirá index.html i sve iz games/ foldera
  let urlPath = req.url.split('?')[0]; // ukloni query string
  let filePath;
  if(urlPath === '/' || urlPath === '/index.html'){
    filePath = path.join(__dirname, 'index.html');
  } else if(urlPath.startsWith('/games/')){
    filePath = path.join(__dirname, urlPath.slice(1)); // ukloni vodeći /
  } else {
    res.writeHead(404); res.end('Not found'); return;
  }
  fs.readFile(filePath, (err, data) => {
    if (err) { res.writeHead(404); res.end("Fajl nije pronađen: " + urlPath); return; }
...
function getRoomList() {
  return Object.entries(rooms).map(([id, r]) => ({
    ...
    maxPlayers: r.maxPlayers, gameActive: r.gameActive, roundTime: r.roundTime,
    gameType: r.gameType || 'geo',
  }));
}

function broadcastRoomList() {
  const all = getRoomList();
  broadcastGlobal({ type: "room_list",          rooms: all.filter(r=>r.gameType!=='imposter') });
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (raw) => {
    ...
    if (data.type === "imposter_reveal_req") {
      ...
      broadcastRoom(room, { type:'imposter_reveal', word:room.imposterGame.word, imposter:room.imposterGame.imposter, votes:room.imposterGame.votes });
