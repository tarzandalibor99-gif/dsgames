...
function getRoomList() {
  return Object.entries(rooms).map(([id, r]) => ({
    ...
    gameType: r.gameType || 'geo',
  }));
}

function broadcastRoomList() {
  const allRooms = getRoomList();
  const geoRooms = allRooms.filter(r=>r.gameType!=='imposter');
  const impRooms = allRooms.filter(r=>r.gameType==='imposter');
  broadcastGlobal({ type: "room_list", rooms: geoRooms });
  broadcastGlobal({ type: "imposter_room_list", rooms: impRooms });
}
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (raw) => {
    ...
    if (data.type === "create_room") {
      ...
      console.log(`🏠 ${myUser} napravio sobu: ${rooms[id].name} (${rooms[id].gameType})`);
      broadcastRoomList();
    ...
    if (data.type === "imposter_join_room") {
      ...
      myRoomId = data.roomId;
      if (!room.players[myUser]) {
        room.players[myUser] = { ws, online: true, locked: false };
      } else {
        room.players[myUser].ws = ws;
        room.players[myUser].online = true;
      }
      recalcRoomAdmin(room);
      const isAdmin = myUser === room.adminUser;
      sendTo(ws, {
        type: 'imposter_joined', roomId: room.id, roomName: room.name,
        isAdmin, adminUser: room.adminUser,
      });
      const playerList = Object.keys(room.players).filter(u=>room.players[u].online)
        .map(u=>({ username:u, online:true, isAdmin:u===room.adminUser }));
      broadcastRoom(room, { type:'imposter_players', players:playerList, adminUser:room.adminUser });
      broadcastRoomList();
      // Pošalji listu imposter soba
      const impRooms = getRoomList().filter(r=>r.gameType==='imposter');
      broadcastGlobal({ type:'imposter_room_list', rooms:impRooms });
      console.log(`🕵️ ${myUser} ušao u imposter sobu: ${room.name}`);
    }

    if (data.type === "imposter_start") {
      const room = myRoomId ? rooms[myRoomId] : null;
      if (!room || myUser !== room.adminUser) return;
    ...
    if (data.type === "imposter_vote") {
      const room = myRoomId ? rooms[myRoomId] : null;
      if (!room || !room.imposterGame) return;
      room.imposterGame.votes[data.voter] = data.target;
      broadcastRoom(room, { type:'imposter_vote_update', votes: room.imposterGame.votes });
      console.log(`🗳️ ${data.voter} glasao za ${data.target}`);
    }

    if (data.type === "imposter_reveal_req") {
      const room = myRoomId ? rooms[myRoomId] : null;
      if (!room || myUser !== room.adminUser || !room.imposterGame) return;
      broadcastRoom(room, {
        type: 'imposter_reveal',
        word: room.imposterGame.word,
        imposter: room.imposterGame.imposter,
        votes: room.imposterGame.votes,
      });
      room.gameActive = false;
      broadcastRoomList();
      console.log(`🎭 Imposter otkriven: ${room.imposterGame.imposter}`);
    }
