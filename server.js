...
let lockedGames = { geo: false, imposter: false, roulette: false };
...
function broadcastRoomList() {
  ...
  broadcastGlobal({ type: "room_list",          rooms: all.filter(r => r.gameType !== 'imposter' && r.gameType !== 'roulette') });
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (raw) => {
    ...
    if (data.type === "chat_msg") {
      const msgData = { type: "chat_msg", username: data.username, msg: data.msg, time: new Date().toLocaleTimeString('sr',{hour:'2-digit',minute:'2-digit'}) };
      const room = myRoom();
      if (room) broadcastRoom(room, msgData);
      else broadcastGlobal(msgData);
    }
